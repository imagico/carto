...
tree.StyleObject = function(name, attachment, definitions, env) {
    var existing = {};
    var image_filters = [], image_filters_inflate = [], direct_image_filters = [], gmic = [], gmic_after = [], comp_op = [], opacity = [];
    ...
    for (var i = 0; i < definitions.length; i++) {
        for (var j = 0; j < definitions[i].rules.length; j++) {
            ...
            if (definitions[i].rules[j].name === 'gmic' && !isNone) {
                gmic.push(definitions[i].rules[j]);
            }
            if (definitions[i].rules[j].name === 'gmic-after' && !isNone) {
                gmic_after.push(definitions[i].rules[j]);
            }
    ...
    if (gmic.length) {
        _.set(styleAttrs, 'gmic', gmic[0].value.ev(env).toString());
    }

    if (gmic_after.length) {
        _.set(styleAttrs, 'gmic-after', gmic_after[0].value.ev(env).toString());
    }
